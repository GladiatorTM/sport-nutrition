...
<div class="min-h-screen py-12 bg-gradient-to-br from-yellow-100 via-blue-100 to-white flex flex-col items-center">
    <div class="max-w-6xl w-full">
        <h1 class="text-4xl font-extrabold text-yellow-500 mb-8 text-center drop-shadow">ДОБАВКИ</h1>
        @include('components.product-filter')
        <div class="flex flex-wrap gap-2 justify-center mb-8">
            <a href="{{ route('categories.dobavky') }}" class="px-4 py-1 rounded-full font-bold text-xs transition {{ request('category') ? 'bg-yellow-100 text-yellow-600 border border-yellow-400 hover:bg-yellow-200' : 'bg-yellow-400 text-white shadow' }}">Всі</a>
        </div>
