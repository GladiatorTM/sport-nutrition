@extends('layouts.app')
@section('content')
<div class="min-h-screen py-12 bg-gradient-to-br from-blue-100 via-yellow-100 to-white flex flex-col items-center">
    <div class="max-w-6xl w-full">
        <h1 class="text-4xl font-extrabold text-blue-500 mb-8 text-center drop-shadow">ВІТАМІНИ</h1>
        @include('components.product-filter')
        <div class="flex flex-wrap gap-2 justify-center mb-8">
            <a href="{{ route('categories.vitaminy') }}" class="px-4 py-1 rounded-full font-bold text-xs transition {{ request('category') ? 'bg-yellow-100 text-yellow-600 border border-yellow-400 hover:bg-yellow-200' : 'bg-yellow-400 text-white shadow' }}">Всі</a>
        </div>
